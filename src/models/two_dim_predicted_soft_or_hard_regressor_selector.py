...
def build_CNN_2D_predicted_soft_or_hard_regressor_selector_model(use_hard_selector=True):
  ...
  if use_hard_selector:
    ...
    one_hot_predicted_batch = tf.keras.layers.Reshape([17, 1])(x_batch)
  else:
    one_hot_predicted_batch = tf.keras.layers.Reshape([17, 1])(outputs_batch)
...
def train_CNN_2D_predicted_soft_or_hard_regressor_selector_model(train_dataset, val_dataset, pretrained_epochs, total_epochs, model_dir, use_hard_selector=True, fine_tune=False):
  if not fine_tune:
